...
The `With(t *testing.T)` function is used to set up a test frame for the current test.  This
...
they accept `any` as the subject type.  This is nessary for matchers which
...
| `opt.AsDeclaration(bool)` | a boolean to indicate whether values (other than strings) in test failure reports should formatted as declarations (`%#v` rather than `%v`) |
