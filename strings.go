...
// //
// //	func(a, b string) bool    // a custom comparison function; if not specified a simple
// //	                          // equality test is used (a == b).
...
// // specified []string; the slices must have the same number of
// // elements in the same order with the same content.
// func (st StringsTest) Equals(want []string, opts ...any) {
// 	cmps := *new(func(string, string) bool)
// 	checkOptTypes(st.T, optTypes(cmps), opts...)
// 	getOpt(&cmps, opts...)
...
// 		if slicesEqual(want, st.got, cmps) {
// 			return
// 		}

// 		st.failWithDiff(t, want)
// 	})
// }

// func (st StringsTest) failWithDiff(t *testing.T, want []string) {
// 	t.Helper()
...
// // fails if the []string being tested is not equivalent to some
// // specified []string; the slices must have the same number of
// // elements with each element occuring the same number of times
// // in each slice, but not necessarily in the same order.
// func (st StringsTest) EquivalentTo(want []string, opts ...any) {
// 	cmps := *new(func(string, string) bool)
// 	checkOptTypes(st.T, optTypes(cmps), opts...)
// 	getOpt(&cmps, opts...)

// 	st.Helper()
// 	st.Run("equivalent", func(t *testing.T) {
// 		t.Helper()

// 		diff, ok := slicesEquivalent(want, st.got, cmps, true)
// 		if ok {
// 			return
// 		}
...
// 				d = "+"
// 			case refs[1] != refs[0]:
// 				d = fmt.Sprintf("%d", refs[1]-refs[0])
// 			}
// 			report = append(report, fmt.Sprintf("| %3s : %q", d, s))
// 		}
// 		var missing []string
// 		for _, s := range want {
// 			refs := diff[s]
// 			if refs[1] == 0 {
// 				d := "?"
// 				missing = append(missing, fmt.Sprintf("| %3s : %q", d, s))
// 			}
// 		}
// 		if len(missing) > 0 {
...
// 		st.errorf(t, strings.Join(report, "\n"))
// 	})
// }
