...
//
// If the options contain a string or opt.SubjectName value, that is used
// as the name of the expectation; otherwise the expectation is unnamed.
...
func (e *expectation[T]) Is(expected T, opts ...any) {
	...
	case any(expected) != nil && internal.IsNil(e.subject):
		// properties of the expectation must be reflected in opts so that
		// the test failure report is handled correctly by Fail
