...
func TestAppendSliceOrArray(t *testing.T) {
	...
	Run(Test("with empty array", func() {
		result := report.AppendSliceOrArray([]string{}, [0]string{}, opt.Name("title:"))
		Expect(result).To(EqualSlice([]string{
			`title: <empty array>`,
		}))
	}))

	Run(Test("with nil slice", func() {
		result := report.AppendSliceOrArray([]string{}, []string(nil), opt.Name("title:"))
		Expect(result).To(EqualSlice([]string{
			`title: <nil>`,
		}))
	}))
