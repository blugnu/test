...
func TestTypeAsString(t *testing.T) {
	...
	Expect(report.Type("test")).To(Equal("string"))
	Expect(report.Type(42)).To(Equal("int"))
	Expect(report.Type(nil)).To(Equal("<nil>"))
	Expect(report.Type(struct{}{})).To(Equal("struct{}"))
	Expect(report.Type([]int{1, 2, 3})).To(Equal("[]int"))
	Expect(report.Type([]any{})).To(Equal("[]any"))
}
