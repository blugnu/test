...
// Equal asserts that the slice under test is equal to the specified values.
// The slices must be of the same length and contain the same values in the
// same order.
func (st SliceTest[T]) Equal(values ...T) {
	test.T().Helper()
	test.Expect(st.got, st.eopts...).To(test.EqualSlice(values), st.mopts...)
}
