...
func TestMap(t *testing.T) {
	...
	Run(HelperTests([]HelperScenario{
		...
		{Scenario: "Equals",
			Act: func() {
				expect.Map(map[string]int{"a": 1, "b": 2}).Equals(map[string]int{"a": 1, "b": 2})
			},
		},
		{Scenario: "Equals fails when maps are inequal",
			Act: func() {
				expect.Map(map[string]int{"a": 1}).Equals(map[string]int{"a": 2})
			},
			Assert: func(result *R) {
				result.Expect(
					"map_test.go",
